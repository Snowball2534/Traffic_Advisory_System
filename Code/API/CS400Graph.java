...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
        ...
        Hashtable<Vertex, Path> table =  new Hashtable<>(); // use hashtable to store the Path for determined node
        queue.add(new Path(startVertex)); // initialize the queue with the start node
        Path workingPath = null;
        while(!queue.isEmpty())
        {
        	workingPath = queue.poll();
        	if(workingPath != null)
        	{
        		table.put(workingPath.end, workingPath);
        		for(Edge e : workingPath.end.edgesLeaving) { // add the successor to queue
        			if(!table.containsKey(e.target)) // if the shortest path has been determined for the node, do not consider again
                    {
        				Path newp = new Path(workingPath, e);
        				boolean flag = false;
        				for(Path p : queue) // if the node has been discovered but not been determined, it needs to update
        				{
        					if(p.end.equals(newp.end))
        					{
        						if(p.distance > newp.distance)
        						{
        							queue.remove(p);
        							queue.add(newp);
        						}
        				...
        				if(flag == false) queue.add(newp);
                    }
        		}
        	}
        }
        if(!table.containsKey(endVertex)) // throw exception when no path between the start node and the end node
        	throw new NoSuchElementException("No path from start to end can be found");
        return table.get(endVertex);
    }
